...
export function ReactPSXSettingsPanel() {
  ...
  return h(
    'div',
    { id: 'psx-settings-panel', className: 'psx-settings-panel ui-card' },
    h('h3', { className: 'psx-settings-title' }, 'Retro Console Graphics'),
    ...
    h(
      ...
      h('input', {
        ...
        onChange: (event: React.ChangeEvent<HTMLInputElement>) => {
          applyPatch({ enabled: event.target.checked });
          sync();
        },
      }),
      h('span', null, 'Enable Retro Mode')
    ),

    h(
      'div',
      { className: 'psx-section' },
      h('label', { className: 'psx-field-label' }, 'Console Preset'),
      h(
        'select',
        {
          className: 'ui-input',
    ...
    h(
      'div',
      { className: 'psx-section' },
      h('label', { className: 'psx-field-label' }, 'Quality Preset'),
      h(
        'select',
        {
          className: 'ui-input',
    ...
    h(
      'div',
      { className: 'psx-section psx-divider' },
      ...[
        ...
        h(
          'label',
          { key: item.key, className: 'psx-check' },
          h('input', {
            type: 'checkbox',
            checked: settings[item.key as keyof SettingsState] as boolean,
            onChange: (event: React.ChangeEvent<HTMLInputElement>) => {
              applyPatch({ [item.key]: event.target.checked } as Partial<SettingsState>);
              sync();
            },
          }),
    ...
    h(
      'div',
      { className: 'psx-section psx-divider' },
      h('h4', { className: 'psx-subtitle' }, 'Color & Lighting'),
      ...sliders.map((slider) => {
        const value = settings[slider.key as keyof SettingsState] as number;
        return h(
          'label',
          { key: slider.key, className: 'psx-slider' },
          h('span', null, `${slider.label}: `, h('span', null, value.toFixed(1))),
          h('input', {
            type: 'range',
            min: slider.min,
            max: slider.max,
