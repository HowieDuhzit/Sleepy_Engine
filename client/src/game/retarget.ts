...
export function retargetMixamoClip(
  ...
  const parentWorldRotations: Record<string, any> = normalizedHumanoid?._parentWorldRotations ?? {};
  const resolveQuat = (value: any) => {
  ...
  for (const track of clip.tracks) {
    ...
    if (!vrmBone) continue;
    const vrmNormalizedNode = humanoid.getNormalizedBoneNode(vrmBone as any);
    const vrmRawNode = humanoid.getRawBoneNode(vrmBone as any);
